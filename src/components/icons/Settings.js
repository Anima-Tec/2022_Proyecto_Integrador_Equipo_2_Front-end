import { theme } from 'config/theme'
import PropTypes from 'prop-types'

const { colors } = theme

export function Settings({ color = 'icon-black', ...rest }) {
  return (
    <svg
      width="30"
      height="30"
      viewBox="0 0 30 30"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...rest}
    >
      <path
        d="M15 18.75C17.0711 18.75 18.75 17.0711 18.75 15C18.75 12.9289 17.0711 11.25 15 11.25C12.9289 11.25 11.25 12.9289 11.25 15C11.25 17.0711 12.9289 18.75 15 18.75Z"
        stroke={colors[color] ?? color}
        strokeWidth="1.5"
        strokeMiterlimit="10"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M2.5 16.1V13.9C2.5 12.6 3.5625 11.525 4.875 11.525C7.1375 11.525 8.0625 9.92504 6.925 7.96254C6.275 6.83754 6.6625 5.37504 7.8 4.72504L9.9625 3.48754C10.95 2.90004 12.225 3.25004 12.8125 4.23754L12.95 4.47504C14.075 6.43754 15.925 6.43754 17.0625 4.47504L17.2 4.23754C17.7875 3.25004 19.0625 2.90004 20.05 3.48754L22.2125 4.72504C23.35 5.37504 23.7375 6.83754 23.0875 7.96254C21.95 9.92504 22.875 11.525 25.1375 11.525C26.4375 11.525 27.5125 12.5875 27.5125 13.9V16.1C27.5125 17.4 26.45 18.475 25.1375 18.475C22.875 18.475 21.95 20.075 23.0875 22.0375C23.7375 23.175 23.35 24.625 22.2125 25.275L20.05 26.5125C19.0625 27.1 17.7875 26.75 17.2 25.7625L17.0625 25.525C15.9375 23.5625 14.0875 23.5625 12.95 25.525L12.8125 25.7625C12.225 26.75 10.95 27.1 9.9625 26.5125L7.8 25.275C6.6625 24.625 6.275 23.1625 6.925 22.0375C8.0625 20.075 7.1375 18.475 4.875 18.475C3.5625 18.475 2.5 17.4 2.5 16.1Z"
        stroke={colors[color] ?? color}
        strokeWidth="1.5"
        strokeMiterlimit="10"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  )
}

Settings.propTypes = {
  color: PropTypes.string,
}
